...
public class RetryPolicy {
	...
	public RetryPolicy(int maxRetries, Set<Class<? extends Throwable>> retryOn, Set<Class<? extends Throwable>> abortOn,
	                   Predicate<Throwable> retryPredicate, Predicate<Throwable> abortPredicate) {
	...
	private static boolean matches(Class<? extends Throwable> throwable, Set<Class<? extends Throwable>> set) {
		return set.isEmpty() || set.stream()
		                           .anyMatch(c -> c.isAssignableFrom(throwable));
	}
	...
	public RetryPolicy retryOn(Class<? extends Throwable>... retryOnThrowables) {
		return new RetryPolicy(maxRetries,
		                       setPlusElems(retryOn, retryOnThrowables),
		                       abortOn,
		                       retryPredicate,
		                       abortPredicate);
	}
	...
	public RetryPolicy abortOn(Class<? extends Throwable>... abortOnThrowables) {
		return new RetryPolicy(maxRetries,
		                       retryOn,
		                       setPlusElems(abortOn, abortOnThrowables),
		                       retryPredicate,
		                       abortPredicate);
	}
	...
	private boolean exceptionClassRetryable(RetryContext context) {
		...
		final Class<? extends Throwable> e = context.getLastThrowable()
		                                            .getClass();
		if (abortOn.isEmpty()) {
